...
from src.llm.lang import (create_collection, ingest_files_in_dir, query,
                          start_client)
...
def main():
    ...
    #         st.write("Generating PPT...")
    #         prs = generate_ppt(client, chat_session_id, list_of_slide_titles, colour_dict)
    #         status.update(label="Done!", state="complete", expanded=False)                  ### this is the with status case, it's special, once all done, can cosider to add in
    #         st.markdown(get_binary_file_downloader_html(prs), unsafe_allow_html=True)
